...
class Location extends Model
{
    ...
    public function scopeJoinAvailability($query) {
        return $query->leftJoin(DB::raw('(SELECT location_id,case when max(doses) = 0 THEN null ELSE min(availability_time) END as available,max(doses) as doses,sum(doses) all_doses FROM availabilities WHERE deleted_at IS NULL AND availability_time >= \''.date('Y-m-d').'\' GROUP BY location_id) as next_availability'), function($q) {
